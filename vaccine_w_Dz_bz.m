...
% Vaccine delay Dz, fixed bz, no restrictions on w, Dx, bz
...
%% choose the fixed parameter and adjust the plotted labels in the end
%bz=1.7;
Dz=0;
%%
...
du=0.1;    
u=0:du:2000; % antigenic coordinates (denoted -u) for K
u2=-2000:du:2000; % a.c. reflected for Kz
% u=0:du:200; % antigenic coordinates (denoted -u) for K
% u2=-200:du:200; % a.c. reflected for Kz
...
%for Dz=[2 1 0.5 0.2 0.1 0.05 0 -0.2 -0.5 -1 -1.5]
for bz=[1.2 1.7 4.1 100]

%% Vaccine crossimmunity and its derivative: added shift by Dz, made symmetric/asymmetric
    Kz = (1+ez/(1-ez)*exp(-abs(u2+Dz)/bz)).^(-1);  % eq. 10
    Kzder = ez/(1-ez)/bz *(2*(u2+Dz > 0)-1).*exp(-abs(u2+Dz)/bz).*(1+ez/(1-ez)*exp(-abs(u2+Dz)/bz)).^(-2);  
...
% Reproduction number
        for k=1:length(cz)
            Re(k)=R0*(1-cz(k)*sqrt(1/(2*pi*w2(k)))*du*sum(exp(-u2.^2/2/w2(k)).*(1-Kz)));  % eq. S37
        end
...
%% Finding full sigma including vaccination: replaced to averaging Kder in i(x)
        for k=1:length(cz)
           Re_der(k)=R0*cz(k)*sqrt(1/(2*pi*w2(k)))*du*sum(exp(-u2.^2/2/w2(k)).*Kzder);  
        end
        sigma=sigma1 + Re_der./Re; 
...
ii=find(sigma>0 & [1, sigma(1:(end-1))] > 0 & Re > 0 );
%ii=find(finf>0 & [1, finf(2:end-1)]>0);
figure(1)
subplot(2,2,1)
    plot(cz(ii), V(ii)/V(1) )
    %% choose parameter on curves bz or Dz
    text(cz(ii(end)),V(ii(end))/V(1),sprintf('%g', bz))
    hold on
    xlabel('Coverage c_z')
    ylabel('Substitution rate')
    %% choose fixed parameter bz or Dz
    title(sprintf('R0=%g, e_z=%g, p_y=%g, b_y=%g, Dz=%g, N=%g, U_b=%g \n %s, bz on curves' ,R0,ez,py,by,Dz,N,Ub,distribution_s ))

    subplot(2,2,2)
    plot(cz(ii), finf(ii)/finf(1))
     %% choose parameter on curves bz or Dz
    text(cz(ii(end)),finf(ii(end))/finf(1),sprintf('%g', bz))
    hold on
    xlabel('Coverage c_z')
    ylabel('Incidence')
    ...
    plot(cz(ii), Re(ii))
     %% choose parameter on curves bz or Dz
    text(cz(ii(end)),Re(ii(end)),sprintf('%g', bz))
    hold on
    xlabel('Coverage c_z')
